...
def fetch_uniswap_price_sync():
    ...
    # token0 = USDC (6 déc), token1 = ETH (18 déc)
    # prix ETH/USDC = token1/token0
...
def generate_signal(price_binance, price_dex):
    ...
    gain_brut = (sell_price - buy_price) * TRADE_SIZE_ETH
    total_fees = TRADE_SIZE_ETH * ((price_binance * BINANCE_TAKER_FEE_BPS / 10000) +
                                   (price_dex * UNISWAP_FEE_BPS / 10000))
    pnl_net = gain_brut - total_fees
    diff_bps = ((sell_price - buy_price) / buy_price) * 10000
    edge_after_fees_bps = diff_bps - (BINANCE_TAKER_FEE_BPS + UNISWAP_FEE_BPS)

    if pnl_net > 0 and edge_after_fees_bps >= THRESHOLD_BPS:
...
Taille indicative: {TRADE_SIZE_ETH} ETH (~{(TRADE_SIZE_ETH*price_binance + TRADE_SIZE_ETH*price_dex)/2:.2f} USDC)"""
    else:
