...
client = Client(API_KEY, API_SECRET, testnet=True)  # ⚠️ testnet=True pour tests

# ⚙️ Paramètres
symbol = "BTCUSDT"
spread = 0.0001       # 0.1% au-dessus/au-dessous du prix
delay = 3            # secondes entre chaque update
quantity = 0.01     # quantité fixe par ordre

print("🚀 Bot Market Making avec PnL corrigé et trace asynchrone...")
...
# ⚠️ On ignore les trades déjà existants
...
try:
    while True:
        ...
        buy_price = round(price * (1 - spread), 2)
        sell_price = round(price * (1 + spread), 2)

        # Annule les ordres ouverts
        open_orders = client.get_open_orders(symbol=symbol)
        for o in open_orders:
            client.cancel_order(symbol=symbol, orderId=o["orderId"])

        # Placement d'ordres limit (sans toucher au PnL ou soldes)
