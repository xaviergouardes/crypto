...
async def main():
    ...
    indicator_swing_detector = IndicatorSimpleSwingDetector(event_bus, lookback=5, history_window=350)

    strategy_engine = StrategySimpleSweepSwingEngine(event_bus)      
    
    #risk_manager = RiskManagerByAtr(event_bus, atr_tp_mult=1.5, atr_sl_mult=1, solde_disponible=1000) 
    risk_manager = RiskManager(event_bus, tp_percent=1.2, sl_percent=0.7, solde_disponible=100) 
