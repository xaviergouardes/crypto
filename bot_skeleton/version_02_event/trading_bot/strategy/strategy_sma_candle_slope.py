...
class StrategySmaCandleSlopeEngine:
    ...
    async def on_indicator(self, event: IndicatorUpdated):
        ...
        # Évaluer la stratégie si possible
        await self.evaluate_strategy()

    async def evaluate_strategy(self):
        """Calcule la pente et publie un signal si le seuil est franchi."""
        if len(self.sma_history) < self.window_size:
            return  # pas assez de points pour calculer la pente

        # Pente sur N ticks : différence entre dernier et plus ancien
