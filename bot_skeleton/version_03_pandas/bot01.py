...
if __name__ == "__main__":
    ...
    # ema_trend_filter = EmaTrendFilter(df, ema_col="ema_21")
    ...
    risk_manager = RiskManager(df, tp_pct=1.5, sl_pct=0.6)
    ...
    # print(filtered_df[['timestamp_paris', 'open', 'close', 'swing_high', 'swing_low', 'sweep_high', 'sweep_low', 'signal']].tail(20))
    # print(filtered_df[['timestamp_paris', 'close', 'signal', 'entry_price', 'tp', 'sl']].tail(20))
    # filtered_df = df[(df['position'] == "CLOSE_BUY_TP") | (df['position'] == "CLOSE_SELL_TP")]
    # print(filtered_df[['timestamp_paris', 'close', 'signal', 'entry_price', 'tp', 'sl', 'position']].tail(20))

    filtered_df = df[df['position'].isin(["CLOSE_BUY_TP", "CLOSE_SELL_TP", "CLOSE_BUY_SL", "CLOSE_SELL_SL"])]
    # filtered_df = df[df['trade_id'].isin((21, 22))]
