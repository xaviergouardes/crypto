...
from trading_bot.core.events import Candle, CandleClose, IndicatorUpdated
...
class IndicatorEmaCrossDetector:
    ...
    async def on_candle_close(self, event: CandleClose):
        # Uniquement la pour récupérer la bougie sur lequel l'aindicateur est valcule
        # devra etre supprimer lorsque on ajoutera la candle dans l'event IndicatorUpdate
    ...
    async def handle_indicator_updated(self, event: IndicatorUpdated):
        ...
        if cross_signal:
            cvs_ligne = self._format_csv_line(cross_signal, self.candle, self.fast_period, self.slow_period)
            self._logger.debug(cvs_ligne)
    ...
    def _format_csv_line(self, signal, candle:Candle, fast_period, slow_period):
        return (
            f"{signal};"
            f"{candle.symbol};" 
            f"{candle.open};"
