...
def strategy():
    ...
    if cycle_count % PRINT_EVERY == 0:
        ...
        print(f"P&L simulé total : {pnl_simule:.8f} {QUOTE} ou {BASE}")
    ...
    if open_position is None:
        ...
        if position_type:
            ...
            if position_type == "LONG":
                print(f"🟢 Signal Achat @ {entry_price:.8f} | SL = {stop_loss:.8f} | TP = {take_profit:.8f} | position_type = {position_type} | stop_level = {stop_level")
            else:
                print(f"🔴 Signal Vente @ {entry_price:.8f} | SL = {stop_loss:.8f} | TP = {take_profit:.8f} | position_type = {position_type} | stop_level = {stop_level")
        ...
        if open_position == "LONG":
            if last_price <= stop_loss:
                ...
                print(f"❌ Stop Loss touché ({last_price:.8f}) -> fermeture LONG | P&L = {pnl_actuel:.8f} {QUOTE} ou {BASE}")
                open_position = None
            elif last_price >= take_profit:
                pnl_simule += pnl_actuel
                print(f"✅ Take Profit atteint ({last_price:.8f}) -> fermeture LONG | P&L = {pnl_actuel:.8f} {QUOTE} ou {BASE}")
                open_position = None
        elif open_position == "SHORT":
            if last_price >= stop_loss:
                pnl_simule += pnl_actuel
                print(f"❌ Stop Loss touché ({last_price:.8f}) -> fermeture SHORT | P&L = {pnl_actuel:.8f} {QUOTE} ou {BASE}")
                open_position = None
            elif last_price <= take_profit:
                pnl_simule += pnl_actuel
                print(f"✅ Take Profit atteint ({last_price:.8f}) -> fermeture SHORT | P&L = {pnl_actuel:.8f} {QUOTE} ou {BASE}")
                open_position = None
...
if __name__ == "__main__":
    try:
        ...
        print(f"P&L simulé final : {pnl_simule:.8f} {QUOTE} ou {BASE}")
